 ...
 */
exports.sendWeeklySummary = (0, scheduler_1.onSchedule)({
    schedule: '0 20 * * 0',
    ...
    for (const shopDoc of shopsSnap.docs) {
        ...
        try {
            ...
            const prevAppointmentRevenue = prevCompletedDocs.reduce((sum, d) => sum + (d.data().totalPrice || 0), 0);
            const prevTips = prevCompletedDocs.reduce((sum, d) => sum + (d.data().tipAmount || 0), 0);
            const prevSalesSnap = await db
                ...
                .where('date', '<=', prevEnd)
                .get();
            const prevSalesRevenue = prevSalesSnap.docs.reduce((sum, d) => sum + (d.data().totalAmount || 0), 0);
